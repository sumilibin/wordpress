...
if ( ! function_exists( 'dairy_farming_header_style' ) ) :	
	function dairy_farming_header_style() {
		$dairy_farming_header_text_color = get_header_textcolor();
		if ( get_theme_support( 'custom-header', 'default-text-color' ) === $dairy_farming_header_text_color ) {
			return;
		}
		?>
		<style type="text/css">
		...
		if ( ! display_header_text() ) :
			...
			.site-title a,
			.site-description {
				color: #<?php echo esc_attr( $dairy_farming_header_text_color ); ?>;
			}
		<?php endif; ?>
		</style>
		<?php
